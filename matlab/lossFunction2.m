% 测试用损失函数,这个损失函数将轮胎位置调整为直线MN与圆的交点到线段MN中点的距离最小
function loss = lossFunction2(theta , X , R , figID , pringtFlag)
    ...
    % * 夹持点到轮胎中心损失
    % loss_dis = 0;
    intersections = lineCircleCross(M , N , [0 , 0] , R);    % 线段MN与圆的交点
    if isempty(intersections)
        % 直接给固定的惩罚值会牺牲梯度, 改为使用圆心到MN中点的距离与R的差值
        center_to_mid = sqrt( mid_MN(1)^2 + mid_MN(2)^2 );
        loss = abs(center_to_mid - R) * 2 + 1000; % 惩罚值
        return; % 提前返回一个很大的loss
    else
